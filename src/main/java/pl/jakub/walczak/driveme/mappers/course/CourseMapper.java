...
//            model.setStudent((Student) student);
        ...
        Set<Long> drivingsToAdd = dto.getDrivings().stream().map(driving -> driving.getId()).collect(Collectors.toSet());
        model.setDrivings(drivingService.findAllById(drivingsToAdd));

        Set<Long> reservationsToAdd = dto.getReservations().stream().map(reservation -> reservation.getId()).collect(Collectors.toSet());
